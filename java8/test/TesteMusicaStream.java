...
public class TesteMusicaStream {
	...
	public static void main(String[] args) {
		...
		// ---------------------------------------------------------------- soma do
		// tempo das musicas
		double somaTempoDasMusicas = musicas.stream().mapToDouble(Musica::getTempo).sum();
		System.out.println("O total do tempo das musicas �: " + String.format("%.2f", somaTempoDasMusicas));
		System.out.println("------------------------------------------------------------------------------");

		// ---------------------------------------------------------------- nome das
		// musicas
		Stream<String> nomesDasMusicas = musicas.stream().map(Musica::getNome).limit(21).distinct();
		...
		// ---------------------------------------------------------------- musicas
		// acima de 6 min
		Map<Boolean, List<Musica>> mapMusicasAcimaDe6min = musicas.stream().filter(m -> m.getTempo() > 5.00)
				.collect(Collectors.groupingBy(m -> m.getTempo() > 6.00));
		System.out.println(mapMusicasAcimaDe6min);
		System.out.println("------------------------------------------------------------------------------");

		// ---------------------------------------------------------------- min
		Optional<Musica> min = musicas.stream().min(Musica::compareTo);
		System.out.println("A musica de menor tempo �: " + min);
		System.out.println("------------------------------------------------------------------------------");

		// ---------------------------------------------------------------- max
		Optional<Musica> max = musicas.stream().max(Musica::compareTo);
		System.out.println("A musica de maior tempo �: " + max);
		System.out.println("------------------------------------------------------------------------------");

		// ---------------------------------------------------------------- quantidade
		// de musicas
		long count = musicas.stream().count();
		System.out.println("H� " + count + " musicas.");
		System.out.println("------------------------------------------------------------------------------");

		// -------------------------------------------------------------
		// Collectors.toMap - Interpretes que come�am com A e suas musicas
		musicas.stream().filter(m -> m.getInterprete().charAt(0) == 'A')
				.collect(Collectors.toMap(m -> m.getInterprete(), m -> m.getNome())).forEach((interprete,
						musica) -> System.out.println("Int�rprete: " + interprete + ", m�sica: " + musica));
		System.out.println("------------------------------------------------------------------------------");

		// -------------------------------------------------------------
		// Collectors.toList - interpretes que come�am com L e suas musicas
		List<Musica> interpretesQueComecamComL = musicas.stream().filter(m -> m.getInterprete().charAt(0) == 'L')
		...
		musicas.stream().filter(m -> m.getTempo() > 4.00).collect(Collectors.toMap(m -> m.getNome(), m -> m.getTempo()))
				.forEach((musica, tempo) -> System.out.println("M�sica: " + musica + " - dura��o: " + tempo));
	}
}
